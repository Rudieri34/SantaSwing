...
namespace TheraBytes.BetterUi.Editor.ThirdParty
{
	...
	public class ReorderableListControl {
		...
		/// </remarks>
		/// <example>
		/// <para>The following listing presents a text field for each list type:</para>
		...
		/// ]]></code>
		/// </example>
		/// <typeparam name="T">Type of type list.</typeparam>
		/// <param name="position">Position of list type.</param>
		/// <param name="item">The list type.</param>
		/// <returns>
		/// The modified value.
		/// </returns>
		...
		/// <summary>
		/// Background color of anchor list type.
		/// </summary>
		public static readonly Color AnchorBackgroundColor;
		/// <summary>
		/// Background color of target slot when dragging list type.
		/// </summary>
		...
		/// <summary>
		/// Position of mouse upon anchoring type for drag.
		/// </summary>
		private static float s_AnchorMouseOffset;
		/// <summary>
		/// Zero-based index of anchored list type.
		/// </summary>
		private static int s_AnchorIndex = -1;
		/// <summary>
		/// Zero-based index of target list type for reordering.
		/// </summary>
		...
		/// <summary>
		/// Zero-based index of type which should be focused.
		/// </summary>
		...
		/// <summary>
		/// Gets the zero-based index of the list type that is currently being drawn;
		/// or a value of -1 if no type is currently being drawn.
		/// </summary>
		/// <remarks>
		...
		/// <summary>
		/// Gets the total position of the list type that is currently being drawn.
		/// </summary>
		/// <remarks>
		/// <para>The value of this property should be ignored for <see cref="EventType.Layout"/>
		/// type events when using reorderable list controls with automatic layout.</para>
		...
		/// <summary>
		/// Gets or sets style used to draw list type buttons (like the remove button).
		/// </summary>
		...
		/// shown above the first list type at the start of the list control.
		/// </summary>
		/// <remarks>
		...
		/// <summary>
		/// Gets or sets a boolean value indicating whether a horizontal line should be
		/// shown below the last list type at the end of the list control.
		/// </summary>
		/// <remarks>
		/// <para>Horizontal line is not drawn for an empty list regardless of the value
		/// of this property.</para>
		...
		/// <summary>
		/// Occurs after list type is inserted or duplicated.
		/// </summary>
		public event ItemInsertedEventHandler ItemInserted;

		/// <summary>
		/// Raises event after list type is inserted or duplicated.
		/// </summary>
		/// <param name="args">Event arguments.</param>
		...
		/// <summary>
		/// Occurs before list type is removed and allowing for remove operation to be cancelled.
		/// </summary>
		public event ItemRemovingEventHandler ItemRemoving;

		/// <summary>
		/// Raises event before list type is removed and provides oppertunity to cancel.
		/// </summary>
		/// <param name="args">Event arguments.</param>
		...
		/// <summary>
		/// Occurs immediately before list type is moved allowing for move operation to be cancelled.
		/// </summary>
		public event ItemMovingEventHandler ItemMoving;

		/// <summary>
		/// Raises event immediately before list type is moved and provides oppertunity to cancel.
		/// </summary>
		/// <param name="args">Event arguments.</param>
		...
		/// <summary>
		/// Occurs after list type has been moved.
		/// </summary>
		public event ItemMovedEventHandler ItemMoved;

		/// <summary>
		/// Raises event after list type has been moved.
		/// </summary>
		/// <param name="args">Event arguments.</param>
		...
		/// <summary>
		/// Indicates whether type is currently being dragged within control.
		/// </summary>
		...
		/// <summary>
		/// Indicate that first control of list type should be automatically focused
		/// if possible.
		/// </summary>
		/// <param name="controlID">Unique ID of list control.</param>
		/// <param name="itemIndex">Zero-based index of list type.</param>
		...
		/// </summary>
		/// <param name="controlID">Unique ID of list control.</param>
		/// <param name="itemIndex">Zero-based index of type which is going to be dragged.</param>
		...
		/// <summary>
		/// Gets a value indicating whether type in current list is currently being tracked.
		/// </summary>
		/// <param name="controlID">Unique ID of list control.</param>
		/// <returns>
		/// A value of <c>true</c> if type is being tracked; otherwise <c>false</c>.
		/// </returns>
		...
		private void DrawFloatingListItem(IReorderableListAdaptor adaptor, float targetSlotPosition) {
			if (Event.current.type == EventType.Repaint) {
				...
				// Fill background of type which is being dragged.
		...
		private void DrawListContainerAndItems(Rect position, IReorderableListAdaptor adaptor) {
			...
			// Position of first type in list.
			float firstItemY = position.y + ContainerStyle.padding.top;
			// Maximum position of dragged type.
			...
			for (int i = 0; i < count; ++i) {
				...
				if (_tracking) {
					...
					// Do not draw type if it is currently being dragged.
					// Draw later so that it is shown in front of other controls.
					if (i == s_AnchorIndex)
						continue;

					// Update position for current type.
				...
				// Did list count change (i.e. type removed)?
				if (adaptor.Count < count) {
					// We assume that it was this type which was removed, so --i allows us
					// to process the next type as usual.
					count = adaptor.Count;
					--i;
					continue;
				}

				// Event has already been used, skip to next type.
...
//*/
		...
		// Command control id and type index are assigned when context menu is shown.
		...
		/// <summary>
		/// Invoked to generate context menu for list type.
		/// </summary>
		/// <param name="menu">Menu which can be populated.</param>
		/// <param name="itemIndex">Zero-based index of type which was right-clicked.</param>
		/// <param name="adaptor">Reorderable list adaptor.</param>
		...
		/// <param name="itemHeight">Fixed height of list type.</param>
		/// <returns>
		/// Required list height in pixels.
		/// </returns>
		...
		/// <summary>
		/// Move type from source index to destination index.
		/// </summary>
		/// <param name="adaptor">Reorderable list adaptor.</param>
		/// <param name="sourceIndex">Zero-based index of source type.</param>
		...
		protected void MoveItem(IReorderableListAdaptor adaptor, int sourceIndex, int destIndex) {
			// Raise event before moving type so that the operation can be cancelled.
		...
		/// <summary>
		/// Add type at end of list and raises the event <see cref="ItemInserted"/>.
		/// </summary>
		/// <param name="adaptor">Reorderable list adaptor.</param>
		...
		/// <summary>
		/// Insert type at specified index and raises the event <see cref="ItemInserted"/>.
		/// </summary>
		/// <param name="adaptor">Reorderable list adaptor.</param>
		...
		/// <summary>
		/// Duplicate specified type and raises the event <see cref="ItemInserted"/>.
		/// </summary>
		/// <param name="adaptor">Reorderable list adaptor.</param>
		/// <param name="itemIndex">Zero-based index of type.</param>
		...
		/// </summary>
		/// <remarks>
		/// <para>The event <see cref="ItemRemoving"/> is raised prior to removing type
		...
		/// </summary>
		/// <remarks>
		/// <para>The event <see cref="ItemRemoving"/> is raised for each type prior to
